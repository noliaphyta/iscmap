// Standalone annotation tool (annotate.html). Everything here is additive:
// it reads data/rooms.geojson and reuses pixelCRS/geoData/floorControl/
// backgroundOverlay/mapConfig exactly as the main app does, but never
// imports or touches roomLayer.js, main.js, or the main app's own
// interactive room polygons. Room colors and notes are stored separately
// in localStorage (isc-map-annotations-v1), exportable/importable as
// their own JSON file - data/rooms.geojson itself is never written to.
...
async function init() {
  try {
    ...
    setStatus("Couldn't load the campus map data. Check your connection and try again.");
    return;
  }
